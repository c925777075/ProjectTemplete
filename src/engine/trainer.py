...
class CustomTrainer(Trainer):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    with context():
                        tr_loss_step, outputs = self.training_step(model, inputs, num_items_in_batch)
                    ...
                    if do_sync_step:
                        ...
                        self._maybe_log_save_evaluate(tr_loss, outputs, grad_norm, model, trial, epoch, ignore_keys_for_eval)
                    else:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, outputs, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            outputs.pop("loss")
            outputs = {k: v.item() for k, v in outputs.items() if torch.is_tensor(v)}
            logs.update(outputs)
